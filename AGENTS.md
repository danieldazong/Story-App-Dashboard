...
**Verified against live storage, not inferred.** Two probes. The round trip minted a URL, `PUT` a chapter containing a redundant heading, CRLF endings, `**bold**`, smart quotes and a five-blank-line run, downloaded it server-side and normalised: `HTTP 200`, heading stripped, CRLF gone, markup and smart quotes intact, blank run collapsed, and a whitespace-only file correctly reported unreadable. The second seeded a script object, ran `deleteBook`, and confirmed the bucket was left clean. Probe objects deleted; the bucket ends at zero.
...
**Verified before shipping**, because gates cannot see runtime failures. 35 probes against `parseFileName`/`evaluateBatch` covering the contract table, allocation collisions, duplicates, ordering and determinism; then a live round trip — mint a signed URL at `<bookId>/<chapterId>/<uuid>.txt`, `PUT` (HTTP 200), download server-side, normalise — confirming the heading stripped, CRLF gone, `**bold**` and smart quotes intact, blank runs collapsed, bucket clean afterwards.
