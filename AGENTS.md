...
**Retry transport failures only, and only in the read path.** `withRetry` in `lib/queries.ts` makes three attempts, backing off 300ms then 900ms, when the message is empty or network-shaped. (It retried *once* after ~250ms until 2026-09-18, which was inside the same cold-connection window as the original failure and so failed identically — see the `RETRY_DELAYS_MS` comment for the measurement.) A permission denial is deterministic — retrying it wastes a round trip and delays an error the operator needs now. It is deliberately **not** installed on the Supabase client's `fetch`: that would silently retry *writes*, and replaying a non-idempotent insert after an ambiguous timeout is how a book ends up with two copies of chapter one.
...
- **Nested scroll container** — ~~the Chapters table must not have its own inner scrollbar inside the page scroll~~. **Reversed for bounded cards (2026-09-19), including the Chapters table itself.** The original rule was written against a table that grew without limit inside the page scroll, where a second scrollbar competed for the same gesture over the same content. The fix it prescribed — let the page own the scroll, sticky header, virtualise — solves the gesture conflict but not the height: a 13-chapter story pushed the danger zone off the screen and a 148-chapter import was unusable. Three tables now window and scroll internally at a fixed height with a sticky header: the Dashboard's `Needs attention` (304px) and `Recent activity` (360px), and the book editor's **Chapters table** (304px). All three are one card among several on a composed screen, so the page scroll still has its own content to move — that is the distinction that matters, not which table it is. Each cuts a row in half at the boundary, deliberately: a container ending flush on a row looks complete and invites no scroll. **What survives of the original rule:** never nest a scroll container inside another scroll container, and never put one on a screen whose entire content is the one list.
- **Overflow menu escapes the card** — the Stories list and Chapters table `⋯` menus must use collision-aware placement so they flip inside the viewport/card on the last rows, rather than rendering past the edge.
...
> **The rename was finished on 2026-09-19, not 2026-09-18.** The first pass renamed what was easy to grep — `label:` breadcrumb values and page `title=` props — and declared itself complete because `grep '"Books"'` came back empty. It had missed every string where the word was lowercase, inflected, interpolated or mid-sentence: the `Book saved` toast, `Search books by title...`, the footer `13 books`, `Delete book`, the `Book details` card heading, `Book not found`, `Browse books`, `Open book`, `Free chapters at start of book`, both list empty states, and roughly a dozen action errors like `That book no longer exists.` **The lesson: a rename is not done when the capitalised exact-match is clean.** Search case-insensitively, for singular and plural, inside template literals and ternaries, and read the prose of every error string — the misses clustered exactly where a grep for the tidy form could not reach.

> **Vocabulary: the UI says Story, the code says book (2026-09-19).** Every user-visible label reads `Stories` / `Story` — the sidebar item, page titles, breadcrumbs, buttons (`New Story`, `Create Story`), empty states and toasts. **Nothing underneath was renamed**: the table is `books`, the columns are `book_id` / `bookId`, the route is `/books/<id>`, the components are `book-editor.tsx` / `books-table.tsx`, and the types are `Book` / `BookRow`. That split is deliberate — a display label is a copy change, a schema rename is a migration with RLS policies, foreign keys, generated types and a mobile app hanging off it.
>
> So in this file: **a quoted or backticked label is what renders and says Story; unquoted prose, identifiers and paths say book and mean the code.** Specs below were written before the rename and their prose still says "book" in the second sense. Do not "fix" a `books` table reference to `stories`, and do not ship a UI string that says Book.

**A1 · Dashboard (work queue)**

Breadcrumb and title "Dashboard". Muted sub-line "What needs finishing." Ember `New Story` top-right.

Three plain stat tiles: Stories, Chapters, Audio coverage. Plain tiles — no sparklines, no trends, no percentage *deltas*. Each carries a computed sub-line (`n published · n draft`, `n free · n locked`, `n chapters still need narration`).

**The third tile is a ratio, not a deficit (2026-09-18).** It read `MISSING AUDIO 20` — the catalog's most important fact stated as a bare negative with no denominator, so 20-of-22 and 20-of-2000 rendered identically. It now reads `AUDIO COVERAGE 2/22` with a 4px proportion bar beneath, matching the ratio format the Stories list has always used (A2). The bar is a readout of one known ratio — no axis, no series, no time dimension, nothing inferred — which is why it does not violate the no-charts rule below. A catalog with zero chapters shows `—`, never `0%` and never `NaN`.

`Needs attention` card: a table of chapter number and title, the missing item as a **labelled status pill** (`Script`/`Narration` warn, `Script and audio` destructive), and a right-aligned muted `Open` button per row. Rows are **grouped under a muted story sub-header**, which replaced a Book column that repeated the same title on every row; the header is suppressed when only one story is in the queue. Titles truncate to one line with a `title` attribute — the cell needs `max-w-0` for `truncate` to work inside a table, or a long title widens the column and breaks the 48px row. Footer count line.

**Both lower cards scroll internally rather than growing (2026-09-19).** `Needs attention` windows at 304px (five rows + sticky header) and `Recent activity` at 360px. Neither caps its rows any more — the ten-row and twelve-group caps were removed when the scroll went in, because hiding entries inside a container that already looks scrollable reads as a bug, and the footer's own "20 chapters need attention" then disagreed with the ten rows above it. The queue's `View all in Stories` link is now unconditional: it leads to the whole catalog, not just the overflow.

`Recent activity` card: timestamped rows, **grouped by local day** (`Today`/`Yesterday`/date) with consecutive same-kind entries collapsed into one expandable row (`6 chapters set locked`). Only *consecutive* entries collapse — a run broken by a different event stays three groups, because the order work happened in is part of what the card is for. The expand affordance is `<details>`/`<summary>`, deliberately: `useState` would make the card a Client Component and forfeit the Suspense streaming that gets the tiles up a round trip sooner.

**No charts, no graphs, no telemetry of any kind.** This screen is a queue, not analytics.

> **Prompt 22 was implemented in part, on purpose (2026-09-18).** `Prompts/22-Dashboard-screen-with-infographics` specified a full infographic redesign: a four-segment readiness strip, per-story completion bars, an access-mix bar and a 14-day activity column chart, plus recharts. Every one of those is individually banned by this file (lines above, plus "Explicitly not in scope" and the UI Quality Bar's "Do not add" list), and that ban records a deletion that already happened once — see "invented telemetry in earlier frames". Rather than silently pick one document over the other, the conflict was raised and the scope was cut to the parts that need no chart and no new dependency: the coverage-ratio tile, the Needs attention rebuild, the activity grouping, and the edge states. **The prompt's diagnosis was correct and its prescription overshot** — the screen's real problem was labelling and grouping, not visual encoding, and three of its strongest sections needed no chart to land. If the catalog grows past ~20 stories the per-story completion card becomes genuinely useful and this decision is worth revisiting; at two stories it would have been redundant with the queue two cards below it. Sections 3, 5, 6 and 0.1 of that prompt remain unimplemented and are the place to start if so.

**A2 · Stories list** (route `/books`, component `books-table.tsx`)

Breadcrumb `Stories`, title `Stories`, ember `New Story`.
...
**A3 · Story editor (create and edit)** (route `/books/<id>`, component `book-editor.tsx`)

One shared component drives both a story's creation and its editing — there is no separate `New Story` screen design. Layout, card order, field order, helper lines, dimensions and spacing are identical across both modes, so an operator sees the whole shape of a book, including its Chapters card, on first visit.
...
| field            | create               | edit           |
| ---------------- | -------------------- | -------------- |
| breadcrumb       | `Stories / New Story`  | `Stories / <title>` |
| page title       | `New Story`          | `<story title>` |
...
Breadcrumb `Stories / <title>` in edit mode (or `Stories / New Story` in create mode), one ember button on the title row.

Two-column 2:1 layout.

Left card **Story details**: Title, Author, Short description (160-char counter), Synopsis (600-char counter), Genres multi-select, Maturity segmented control (General / Mature 18+ — **Mature 18+ dark-filled by default**), **Default chapter access** (Select, Free / Locked — see Project Rules). **Status is not a field on this card in create mode** — see "Create Story dialog" below; in edit mode it reappears as a Select (Draft / Published), unchanged from before, so an operator can still flip an existing book's publication state directly on the card.
...
Below the composer, full width, **Chapters** card. The header row inside the card carries `Import chapters` and `Add chapter`, both primary-outline. **This supersedes the earlier "no bulk-upload button here" rule**, which held only while bulk import lived at an unbuilt A7 address: it is now a real screen at `/books/<id>/import` (prompt 10), and the card's own empty state has promised "or import a folder of scripts" since prompt 05 with nothing behind it. `Import chapters` is disabled in create mode — bulk import needs a saved book to import into, the same constraint `Add chapter` carries — and rendered disabled rather than hidden so the capability stays discoverable. Sub-line is computed at render: `<n> chapters · <n> with text · <n> with narration`.
...
**A7 · Bulk Script Import** _(BUILT, at `/books/<id>/import` — not the address below)_

Dropzone accepting a folder of `.txt` / `.docx` / `.md`. A filename-to-chapter matching preview table with per-row override of number and title. Confirm button.

Without this, seeding an 85–200 chapter serial is not viable — this was previously called the highest-value remaining screen, and that assessment stood until prompt 10 built it.

**It has no target-book select, deliberately.** The screen mounts under the book, so the target is a route parameter rather than a form field. That deletes the original spec's Step 1 card, its `Choose a target book first` disabled state, and the class of error where an operator imports 148 chapters into the wrong serial. Reached from the Chapters card's `Import chapters` button and its empty state, never from a sidebar.

> **Uploads removed from the sidebar.** Both `/uploads` and `/uploads/bulk-import` were empty placeholder pages (a title and breadcrumb, no actual content) — A6/A7 above were "approved, not yet designed/built" specs that never got implemented behind them. The sidebar link, the `Uploads` nav item, both route files, and the two "Use bulk import" links that pointed at the bulk-import placeholder (Manuscript card's too-few-sections message, and the unlinked `/books/<id>/chapters/new` page) were all removed, since a nav item and links pointing at nothing is worse than not having them. The Stories list already shows per-story chapter/audio completion ratios, which covers the "what's been uploaded" need this screen would have shown at a glance. If A6/A7 are built later, re-add the sidebar item and route files together — do not leave a nav item pointing at an empty page again.

### Actionable Rows

In the Story editor's Chapters table, a missing asset is a task, not a label:
...
**Surfaces added:** `/sign-in/[[...sign-in]]` (prebuilt `<SignIn />`, **no general sign-up route** — operators are provisioned in the Clerk Dashboard or by invitation; see the ticket-gated exception below), `/not-authorised` (heading, one muted line, `Sign out`), and `/account/[[...rest]]` (Clerk's `<UserProfile />`, where `Change password` on the Settings Account card now links). The last three sit outside the `(dashboard)` group so they render without the sidebar shell and without tripping the admin gate. The sidebar gained a footer (`components/shell/sidebar-user.tsx`, `<UserButton />` plus the operator's name, `mt-auto` so it sits flush at the bottom); the three nav items above it are unchanged.
...
**One ticket-gated sign-up surface exists, added 2026-09-18 with the Team card.** `/accept-invitation/[[...accept-invitation]]` renders `<SignUp>`, and `proxy.ts` treats it as public alongside `/sign-in`. This narrows the "no sign-up route" rule rather than reversing it: Clerk only completes the flow when the request carries a valid `__clerk_ticket` from an invitation an admin actually sent, so it cannot be used to self-provision. Without it an invitation is undeliverable in practice — two failures proved it, both seen in the browser:

1. **`redirectUrl` must be ABSOLUTE.** It was passing `NEXT_PUBLIC_CLERK_SIGN_IN_URL`, the bare path `/sign-in`, which Clerk resolved against its own `*.accounts.dev` domain — so Accept landed on a 404 on Clerk's host, not this app. `NEXT_PUBLIC_APP_URL` was added for this; **a deployed environment must set it** or every invitation points at localhost.
2. **The target must be the sign-UP surface.** The email's Accept button carries `__clerk_status=sign_up` because the invitee has no account yet, so `<SignIn>` cannot complete it. Fixing only the origin would have moved the same dead end onto this app's domain.
...
Parse server-side, store the extracted text on the chapter row, and keep the original file in `scripts/` for reference. All three hold as of prompt 17:

- **Extraction is server-side**, in `extractAndSetChapterScript` (`app/actions/scripts.ts`). The browser uploads the file to `scripts/` and never parses it; the action downloads from the bucket, extracts, normalises, persists and revalidates. `src/lib/docx.ts` keeps both entry points — the server one for that action, the browser one for the Manuscript card and the create-mode composer, which still split text client-side before any row exists to attach it to.
- **The extracted text IS persisted**, in `script_text`. (An earlier revision of this note said the text "lives only in client state" — that stopped being true at prompt 14.)
- **The original file IS kept**, at `scripts/<bookId>/<chapterId>/<uuid>.<ext>`, with `script_path` populated. A `.docx` whose extraction went badly can now be re-parsed from the source.

**Server-side extraction is a boundary, not a preference.** Parsing in the browser means the rules — normalisation, the empty-file rejection, the format allowlist — are enforced by whichever client happens to run, and a second client (bulk import, a future API) re-implements them or skips them. Doing it in the action makes them structural.

**A rejected extraction deletes its own upload.** If the file yields no readable text the action removes the object it just downloaded before returning the error, so a refusal never leaves an orphan behind a row that does not reference it.
...
**Normalisation is a pure function, and only removes what is unambiguously noise.** `lib/script-normalise.ts` runs on every server-side extraction: CRLF → LF, trailing whitespace trimmed, runs of three or more blank lines collapsed to one, and a redundant leading heading stripped. Everything else is left exactly as authored — `**bold**`, `_italic_`, `## `, smart quotes, em dashes and internal blank lines all survive, because the toolbar's three marks and the reader both depend on them.

**The heading strip is deliberately conservative.** It removes a first line only when that line is *nothing but* the chapter's own identity (`Chapter 7`, the chapter title, or both) — never a line that merely begins with it. A `.docx` whose first line is the chapter title repeated is the common case; a chapter that opens with a sentence starting on the title's words is not noise, and guessing wrong deletes authored prose with nothing to recover it from.

**`hasReadableText()` gates persistence.** A file that extracts to whitespace only is rejected rather than written, so a corrupt or empty upload cannot replace a chapter's text with nothing. Verified with 22 probe cases before being wired — the same discipline the markup parser's 19 cases exist for, and for the same reason: offset and whitespace logic typechecks cleanly while being wrong.
...
`books` carries a `default_chapter_access` column (`chapter_access` enum, `not null default 'locked'`), **migrated in prompt 13** (`20260917000001`) — an earlier revision of this note called it "planned but not yet migrated", which stopped being true then. Every chapter created inside a book — manually, by manuscript split, or by bulk import — inherits this value unless `app_settings.free_chapters_at_start` places the chapter inside the free run at the start of the book.

That rule is implemented in two places and they must not drift: `createChapter` (`app/actions/chapters.ts`) resolves explicit access → book default → `app_settings`, and `createImportTarget` (`app/actions/bulk-import.ts`) resolves `number <= freeChaptersAtStart ? "free" : <book default>`. Bulk import resolves the free run itself because it inserts rows directly rather than calling `createChapter` — it needs the chapter id back to mint an upload path against. If either rule changes, change both.
...
1. **Rotate the Clerk secret key — TWICE OVER NOW, and the production one is the urgent half (2026-09-19).**

   **The PRODUCTION `sk_live_` key is compromised.** It was pasted into a screenshot during the production cutover and is now in an assistant transcript — the same mistake as the original 2026-09-16 exposure, repeated on a key that guards the live site. Rotate it in this order, which is not optional: Clerk (Production) → API keys → **+ Add new key** → copy it with the **copy button, never the reveal eye** → update `CLERK_SECRET_KEY` in Vercel → **redeploy** → confirm sign-in still works → **then** delete the old key. Deleting first leaves a window with no working key.

   > **The rule this earns: never reveal a secret on screen while a session is being screenshotted.** Both exposures happened because the value was made visible in order to copy it. Every dashboard that shows a secret also has a copy-to-clipboard button; use that one. A key that is never rendered cannot be captured.

   **The original development-instance task, still open:**

   **Rotate the Clerk secret key.** The current `CLERK_SECRET_KEY` in `.env` was exposed in an assistant conversation on 2026-09-16 (read out of the file into a chat transcript). `.env` is gitignored and was confirmed untracked, and the key showed "Never used" in the Clerk Dashboard at the time, so there is no evidence of misuse — but it has left its intended home and must be replaced. Clerk has no in-place "regenerate": API keys → **+ Add new key** → put the new value in `.env` → **then** delete the old `default` key (in that order, or there is a window with no working key).
...
3. **Provision a production Clerk instance.** — **DEFERRED BY DECISION, 2026-09-19. Not a pending task.** The cutover was completed, verified, and then rolled back when the sign-in card stopped rendering (below). Asked whether to keep chasing it, the answer was to stay on the development instance: **this dashboard is in-house**, and every symptom of a dev instance is external-facing — invitation emails landing in spam, `[Development]` in their subject lines, and the dev-mode badge under the avatar. A handful of operators who can be told "check your spam folder" is a smaller cost than an unresolved rendering failure on the screen they sign in through.
   ...
   **Do not re-test bot protection.** It was disabled with no effect, and the development instance runs the same Smart CAPTCHA while rendering correctly — so it was never a plausible cause. That check was available from the start and was run only after the setting had been changed, which is the same "theorise before gathering evidence" failure this file's Debugging Playbooks already record. Everything reachable by `curl` was healthy throughout; that proved only that the fault was somewhere `curl` cannot see.

   **Originally recorded as DONE 2026-09-19** on the strength of a working sign-in and a Dashboard rendering real counts. That verification was real but did not hold.

   **Was: DONE 2026-09-19.** `talebrim.com` runs on a Clerk **production** instance: DNS verified (all five CNAMEs incl. `clkmail`), certificates issued, `https://clerk.talebrim.com` registered as a second Supabase Third-Party Auth issuer beside the dev one, Clerk's Supabase integration enabled, the session-token claim set, and `pk_live_`/`sk_live_` swapped in Vercel. Verified in the browser: sign-in works, the "Development mode" badge is gone, and **the Dashboard renders real counts** — which is the check that proves the issuer registration landed, since a wrong issuer signs in fine and then reads nothing. Full procedure, the two traps hit, and the rollback are in `docs/PRODUCTION-CLERK-CUTOVER.md`.

   **Two traps worth knowing before repeating this.** Cloning the dev instance **fails on the Hobby plan** — it tries to carry "Hide Clerk branding", which is paid on production — so a **default** instance was created instead, and a default instance carries *none* of the settings the app depends on (session-token claim, Supabase integration, application name). And **Vercel locks an environment variable's Type at first save**: `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` cannot be saved as `Secret` (Vercel refuses a `NEXT_PUBLIC_` prefix marked secret, correctly — Next inlines it into the browser bundle), and a variable already saved as Secret can only be moved to Config by deleting and re-adding it.

   The original blocked note follows, kept for the four-symptoms table which is still the best record of what a development instance looks like from the outside.

   **Was: BLOCKED, deliberately — no domain owned and no deploy planned as of 2026-09-18.** Verified that day: the instance reports `environment_type: "development"` with one domain, `cheerful.walleye-3066.lcl.dev` (Clerk's auto-generated dev domain), and the repo has no deploy config at all — no `vercel.json`, no `netlify.toml`, no Dockerfile.
...
5. **Re-run prompt 11's verification against live sessions.** ~~Confirm a signed-in user *without* `role: admin` cannot reach `/`, `/books` or `/settings`~~ — **done 2026-09-18.** Verified with two real Clerk users against the development instance: the non-admin was redirected to `/not-authorised` on all three routes. `requireAdmin()`'s redirect path is confirmed working.
   ...
   **Re-verified in production against a real stranger, 2026-09-19.** A genuinely uninvited account signed up through the live site and landed on `Not authorised` — *"This account doesn't have operator access to the Talebrim dashboard."* Every earlier run of this test used deliberately-created test users on the development instance; this one was the real condition the guard exists for.
   ...
   **The version that works** keeps `clerkMiddleware` mounted so `auth()` retains its request context, and disables only the guarding:
   ...
   export default clerkMiddleware(async (auth, request) => {
     if (!isPublicRoute(request)) {
       // await auth.protect();   ← comment out, restart, retest, restore
     }
   });
   ```
...
**Correction (2026-09-17).** The paragraph that stood here said prompts 11–18 "have not been implemented — there is no backend, no auth and no real file upload anywhere in this app yet", that every server action was "stubbed as a typed no-op", and that `default_chapter_access` existed "on the mock `Book` type and fixture only". Every one of those statements is now false. Prompts 11–17 all landed (Clerk auth, schema/RLS, reads, writes, cover upload, audio TUS upload, script upload/parse), the mock fixtures were deleted in prompt 13, and prompt 10 — rewritten and implemented on 2026-09-17 — closed bulk import. **Prompt 18 was superseded, not implemented**: it was a wiring prompt for a screen prompt 10 never built, so its scope folded back into the rewritten prompt 10. Its file carries a SUPERSEDED banner explaining what was and was not carried over.

Treat writes in this app as real. The honesty caveats scattered through the notes below — "nothing reaches a database", "validates and toasts without persisting" — describe states this codebase has left, and are kept only because they record *why* each surface was built the way it was.
...
**Uploads longer than a minute failed partway, from an expired token (2026-09-23).** A narration file under the 50 MB ceiling failed with the card's catch-all *"Upload failed. Check your connection and retry."* Cause: the Clerk session token was fetched **once** and replayed as a static `headers.authorization` on every 6 MB chunk. Clerk session tokens live **60 seconds** — the same lifetime prompt 12's storage probes recorded — so any transfer lasting longer than that began sending an expired token mid-upload and Storage rejected the next PATCH. **The failure scaled with upload time, not file size**, which is why "it's under 50 MB" did not rule it out. Fixed with `onBeforeRequest`, which calls `getToken()` per request; Clerk returns its cached token and mints a fresh one only near expiry, so the per-chunk cost is negligible. A null token throws `session expired` rather than sending an unauthenticated request.

**Three defects in the error mapping hid it, and each is worth recognising again:**

- **The fallback named the one cause already ruled out.** It read *"Check your connection"* — but network failures have their own branch immediately above it, so reaching the fallback meant the network was *not* the problem. It now states the HTTP status (`storage returned 403`) and points at the console, where `onError` logs the full error every time.
- **The response body was never read.** `String(error.originalResponse)` is `"[object Object]"`, so every reason Storage put in the body was invisible to the matching. It now reads `originalResponse.getBody()`.
- **The word `expired` matched the wrong branch.** A `jwt expired` body would have been reported as *"Upload link expired after 24 hours"* — right word, wrong cause, wrong remedy. Auth is now matched first, and a row-level-security denial is split from an expired token: both arrive as 403, and only the second is fixed by retrying. Status codes are now matched on the **number**, not by scanning the text, which also contains URLs and base64 upload ids that can hold `409` or `410`.

**That first fix was necessary and not sufficient — same day, second attempt.** The upload still failed, now with the correct *"sign-in lapsed"* message. The dev overlay's call stack put the failure at tus `upload.js:699`, the **chunk `PATCH`**, so creation had succeeded and a later chunk was refused on auth despite a fresh `getToken()` per request. Measured rather than guessed: clock skew **+1.7s** (ruled out), and one 6 MB chunk takes **~5s** to reach Cloudflare's nearest edge from this machine, longer to `us-east-1`. The gap is that `getToken()` keeps serving its **cached** token until ~10 seconds before expiry, so a chunk can start holding a token with seconds to live. Each request now decodes the token's `exp` and forces `getToken({ skipCache: true })` below 45 seconds, so every chunk starts with most of a minute. A failed refresh falls back to the cached token rather than failing the chunk.

**Verified in the browser, 2026-09-23.** After the second fix, a **29.7 MB `.wav` (10:48)** uploaded end to end with its duration detected — the size of file that had been failing. At ~5 seconds per 6 MB chunk that transfer runs well past the 60-second token lifetime, so it exercises exactly the path that broke. **Two attempts were needed, and the order is the lesson:** the first fix (a token per request) was reasoned from the code and was incomplete; the second came from measuring skew and chunk time and reading which request the stack said had failed. Measure first.

**One inconsistency left in place, deliberately.** The card's constraint line advertises `.aac` because an operator added it in Settings, but the `audio` bucket's `allowed_mime_types` does not include it and `MIME_BY_EXTENSION` has no entry for it. An `.aac` file is refused **before** upload with a specific message (*".aac is configured in Settings but not supported for upload yet"*), so nothing breaks silently — but the copy promises a format the platform refuses. Resolve it one way or the other: add `audio/aac` to the bucket and the map, or remove `.aac` from Settings.

**The first diagnostic log was useless, and that is its own lesson.** It logged `{ status, error }`, and the Next.js dev overlay rendered it as **`{}`** — an `Error` serialises to nothing — so the one line added to capture evidence captured none the first time it fired. It now logs a single plain string: which request (`POST`/`PATCH`/`HEAD`), the status, **how many seconds the token had left when sent**, and the response body. That last field is what separates "the token expired" from "a valid token was refused", which otherwise look identical. The token itself is never logged. **Log diagnostics as strings, not objects, anywhere a dev overlay or error reporter may serialise them.**

Probed with 11 cases (both known expired-token phrasings, RLS, a null session, 413, 415, 404, 409, offline, an unknown 500, and a URL containing `409`/`410`), all passing, and `tokenSecondsLeft` with 5 more (fresh, near-expiry, expired, missing `exp`, garbage — including base64url payloads containing `-`/`_` with padding stripped). **The expired-token response wording is assumed, not captured** — the first real failure after this fix will log the actual body to the console, and if it reads differently the auth branch's pattern should be widened to match it.

**`deleteBook` no longer orphans storage.** It read nothing before deleting, and chapters cascade via the foreign key the instant the book row goes — so every audio path became unrecoverable at exactly the moment it was needed. It now collects `cover_path` and every chapter `audio_path` **before** the delete (one `Promise.all`), then removes the objects after the row, tolerating and logging a failed delete rather than failing the action: the book *is* gone, and reporting failure would claim otherwise. Verified end to end against live storage — seeded a real cover and audio object, deleted the book, confirmed both objects removed. `scripts` was deliberately left out at the time, since nothing wrote to that bucket yet and speculative cleanup for an unpopulated path is worse than none. **Prompt 17 populated it and extended the same collect-then-delete pass to cover it** — verified the same way.

**Prompt 17 (Script upload and parse)** moved extraction off the client and started keeping the source file. `app/actions/scripts.ts` holds four actions: `createScriptUploadUrl` (validate format and size, mint a signed URL under `scripts/<bookId>/<chapterId>/`), `extractAndSetChapterScript` (download, extract, normalise, persist `script_text`/`script_path`/`script_file_name`, revalidate), `discardScriptUpload` and `removeChapterScript`. The mechanics of normalisation and the server-side boundary are under **Upload Rules → Scripts** and **Script formatting and the preview pane**; not restated here.

**`ScriptAsset` gained a `path`, which made a real state representable for the first time.** 38 existing chapters carry `script_text` with no `script_path` — text typed or pasted directly, or extracted client-side before this prompt. Without `path` on the type, "has text" and "has a source file" were the same bit, so those rows would have rendered a `Replace file` button over a file that does not exist. They now show `Choose file` instead, and the card distinguishes the two cases by construction rather than by a comment asking the next reader to remember.

**Two controls were dead when first written, and every gate passed.** `Replace` and `Remove` set state that nothing rendered — the confirmation dialogs did not exist yet — so both buttons were inert. Typecheck, lint and build were all clean, because a `useState` setter with no consumer is valid code. Caught by walking the states the card can actually reach rather than by trusting the gates. This is the same class as the Manuscript card's unreachable file input (see *Leave a way out of every terminal state*): **a control that sets state is not a control until something renders that state.**

**`acceptedScriptFormats` now comes from `app_settings`**, threaded through all three script cards and their five call sites, replacing the `SETTINGS_DEFAULTS` constant each read locally. That closes the same gap flagged against `NarrationAudioCard` in prompt 13 — an operator editing accepted formats in Settings now sees every constraint line and every `accept=` attribute change together.

**The file name renders from the server row, not from form state.** An upload extracts and persists server-side and then revalidates, so pushing the extracted text back through react-hook-form would mark the form dirty over work already saved — and leave `Save chapter` enabled with nothing to write. Same reasoning that took narration out of `audioDirty` in prompt 16. `scriptFileName` stays in the schema and in the submit payload so an ordinary Save still preserves it.

**Partly verified against live storage — and reported as more than that, which was wrong.** Two probes. The round trip minted a URL, `PUT` a chapter containing a redundant heading, CRLF endings, `**bold**`, smart quotes and a five-blank-line run, downloaded it server-side and normalised: `HTTP 200`, heading stripped, CRLF gone, markup and smart quotes intact, blank run collapsed, whitespace-only correctly reported unreadable. The second seeded a script object, ran `deleteBook`, confirmed the bucket left clean. Both genuinely passed.

> **Correction (2026-09-17).** This paragraph originally opened *"Verified against live storage, not inferred"*, and that claim was false for the case that mattered most. **Both probes used a `.txt` file.** `extractAndSetChapterScript` branches on extension, and only the `.docx` arm calls `extractDocxTextFromBuffer` — so the probe never executed it, and **every `.docx` upload through this action threw**. The cause: `mammoth.extractRawText({ arrayBuffer })` is browser-only. mammoth's `browser` field swaps `lib/docx/files.js` for a browser build, and only that build reads `arrayBuffer`; the Node build takes `path` or `buffer` and rejects anything else with `Could not find file in options`. Fixed by passing `{ buffer: Buffer.from(...) }`, verified against mammoth's own fixtures and a live upload.
>
> **The rule this earns: a probe that exercises one branch of a format switch verifies that branch and nothing else.** `.txt` and `.docx` take different code paths by design, so a `.txt` probe is not evidence about `.docx` — and calling it "verified end to end" made a stronger claim than the evidence supported. Probe every branch a switch can take, or name the branch that was actually tested.
>
> It stayed hidden for two prompts because the call site wrapped extraction in a bare `catch {}`, which discarded `Could not find file in options` and re-surfaced it as a generic "text not imported" — reading like a bad document rather than a broken call. **Both call sites now `console.error` the real exception.** See *Debugging Playbooks*: an error you swallow is an error you will diagnose by guessing.

**Prompt 10 (Bulk script import)**, implemented 2026-09-17, out of sequence and rewritten first. Four new files: `lib/bulk-import.ts` (pure parsing and conflict evaluation), `app/actions/bulk-import.ts` (four actions), `books/[bookId]/import/page.tsx` (server shell), `components/books/bulk-import-screen.tsx` (the workflow). One edit to existing code: two entry points in the Chapters card.

**The prompt was rewritten before implementation because most of it no longer described this app.** It specified `data/mock-catalog.ts` (deleted in prompt 13), forbade creating any Supabase client or Server Action, and told the implementer to mark `.docx` rows unreadable pending approval of `mammoth` — approved and shipped in prompt 17. The rewrite is recorded in the prompt file itself.

**Prompt 18 was superseded rather than implemented, and that was the substantive finding.** It opens *"Study the existing Bulk script import screen (`app/(dashboard)/uploads/bulk-import/page.tsx`), the pure parser in `lib/bulk-import.ts`"*. Neither existed: prompt 10 was never built, and `/uploads` had been removed from the sidebar along with both route files. A grep for `bulk-import|bulkImport|/uploads` across `src/` returned zero matches. Implementing 18 as written would have meant wiring up a screen that was not there. **Check that a prompt's anchors exist before starting it** — the build's own route listing settled this in seconds.

**Two of prompt 18's instructions were deliberately not carried over.** It asked for a shared `lib/script-text.ts`; `lib/script-normalise.ts` already *is* that module and `lib/docx.ts` already exposes both a browser and a server entry point, so creating a third would have been the duplication the instruction existed to prevent. It also required virtualising the preview table, which needs `@tanstack/react-virtual` — not installed, and AGENTS.md forbids installing without asking. Batches are capped at 150 files instead. **Raise that cap only alongside real virtualisation.**

**Bulk and single-file import share one pipeline, by construction.** `ingestImportedScript` calls the same `extractDocxTextFromBuffer`, the same `normaliseScriptText`, and the same `hasReadableText` gate as `extractAndSetChapterScript`, so the same file produces byte-identical `script_text` through either path. This is the reason prompt 17 moved extraction server-side: a second client would otherwise re-implement the rules or skip them.

**The client and the server call the same `evaluateBatch`.** The preview renders it locally; `preflightBulkImport` re-runs it against freshly-read chapter numbers and returns authoritative rows. When the two disagree — another operator added chapters while the files sat in the dropzone — the screen shows the corrected statuses and **imports nothing on that call**. A preview that disagrees with what the import does is the defect this design exists to prevent.

**One activity entry per batch, not one per chapter.** Per-row actions skip `revalidatePath` entirely; `finalizeBulkImport` is the batch's only cache invalidation and its only log write. 148 rows would otherwise mean 148 log entries and 148 revalidations. It runs even when rows failed, and even when the batch aborted — rows that already settled changed the book, and the screens must reflect them.

**Partial success is designed for, not tolerated.** A row whose chapter is created but whose extraction fails leaves a real chapter with `script_text` null. It is never rolled back: the chapter appears in the Dashboard's Needs attention queue and is fixable in the Chapter editor, and the failure copy says so — *"Chapter created, text not imported. Fix in the chapter editor."* Deleting it would discard a row the operator may have already edited.

**An RLS rejection aborts the batch; a per-row failure never does.** A permissions failure will fail identically for all 147 remaining rows, so stopping is kinder than 147 more identical errors. Everything else — a bad file, a duplicate number, a dropped connection — fails its own row and lets the batch continue.

**Probed before shipping**, because gates cannot see runtime failures. 36 probes against `parseFileName`/`evaluateBatch` covering the contract table, allocation collisions, duplicates, ordering and determinism; then a live round trip — mint a signed URL, `PUT` (HTTP 200), download server-side, normalise — confirming the heading stripped, CRLF gone, `**bold**` and smart quotes intact, blank runs collapsed, bucket clean afterwards.

**That round trip used `<uuid>.txt`, and inherited prompt 17's blind spot exactly.** The `.docx` arm was never executed here either, so bulk import shipped with the same broken mammoth call — see the correction under prompt 17 for the mechanism and the rule. Both were fixed together.

**Four defects survived every gate and were found in the browser, three of them in the preview table.** Worth listing, because they share a shape:

1. **`no_number` was a dead end.** A file whose name carried no number was allocated one, that number was rendered into an editable input, and the row then reported *No number found* with its checkbox disabled — the screen showing a resolved number while the status denied it, and the only escape being to retype the number already on display. The status was **deleted**, not patched: an allocated number continues from the book's highest chapter and skips anything claimed, so it *is* a resolution.
2. **A non-global regex.** `SEPARATORS` had no `g` flag, so `String.replace` substituted one separator and left the rest — `Say_It_Before_You_Stop_Trying` became `Say It_Before_You_Stop_Trying`, which reads as a truncated title rather than an unreplaced one.
3. **Every row arrived unchecked**, so the button permanently read `Import 0 chapters`. The auto-check lived in a `useEffect` over the evaluated rows, guarded by a "have I considered this row?" ref so an operator's unchecking would not be undone — but the guard marked rows considered on a pass where nothing was checked. Fixed by moving the decision into `addFiles`, the one moment the question has an unambiguous answer: the row did not exist a line ago, so nothing can have decided about it. The effect and the ref were deleted.
4. **`JWT not yet valid`** on clicking Import — the dev machine's clock running 2–3s behind Supabase, so Clerk minted tokens whose `nbf` was in the future. Three separate faults: `withRetry` did not treat it as transient, `classifyDbError` filed it under `unknown` (whose branch returns `error.message` raw, which is why Postgres text reached the screen), and `QueryErrorCard` had no guidance for it. Now a `skew` kind, classified **before** the network test, retried on a longer delay than a network blip because the wait must outlast the clock difference, with copy naming the actual remedy.

> **The shape they share.** Every one typechecked, linted and built clean. A dead-end status, a missing regex flag, a state guard that eats its own initial decision, and an error class that falls through to a raw-message branch are all *valid code*. This is the fourth entry in this file making the same point (see also prompt 17's dead controls and the Manuscript card's unreachable input) — **the gates prove the code compiles, never that it does the thing.** Walk the states a screen can actually reach.

> **Two probe failures were the probe's fault, not the code's, and saying so mattered.** One compared objects with `JSON.stringify`, which is key-order sensitive — all three allocated numbers were correct. The other asserted that `The 39 Steps.txt` parses to chapter 39, under a heading reading *"must NOT find a number"*. The code was right: digits must be **leading** to count, so that file is an unnumbered chapter titled "The 39 Steps". The temptation in both cases is to edit the expectation until it matches the output; the discipline is to work out which of the two is wrong first, and say which.

**Three defects were caught by reading the new code rather than by any gate**, all of which typechecked cleanly: a `useRef` referenced by a `useEffect` declared above it (a TDZ throw at runtime — `const` is not hoisted), a progress denominator computed from settled rows rather than batch size (`3 of 3` with nine rows still queued), and a `<Link>` used without its import. The first two are the same family as prompt 17's dead controls: **valid TypeScript, broken page.**

**Prompt 19 (States pass)**, implemented 2026-09-18, closes the series. No new features: every screen already worked on the happy path, and this made them behave when data is absent, slow or broken. Verified in the browser across all five of its runtime checks.

**The prompt was rewritten first**, like 16, 17 and 10 before it, because three of its seven sections described an app that no longer existed — `/uploads` and `/uploads/bulk-import` boundaries for deleted routes, in-page skeletons that had already been replaced, and a `New book` action renamed app-wide. The rewrite is recorded in the prompt file.

**`retry()`, not `reset()` — and the existing boundary was on the wrong side of it.** On next 16.3.5 both props exist and they are not interchangeable: `reset()` clears error state and re-renders *without re-fetching*, so against a Supabase read that is still failing it renders straight back into the same error and reads as a dead button. `retry()` re-fetches. `(dashboard)/error.tsx` predated this prompt, used `reset()`, and carried a comment asserting that `reset()` "re-renders the segment" — describing the wrong prop's behaviour for this version. Migrated, comment corrected. **Confirmed in the browser**: broke `NEXT_PUBLIC_SUPABASE_URL`, loaded the boundary, restored the URL, and pressed `Try again` *without reloading* — the books loaded, which only a re-fetch can do.

**Six boundaries share one `RouteErrorCard`** rather than existing as six near-identical files that would drift. What each must get right lives there once: `error.message` never rendered (Next redacts it in production anyway, and in development it reads as a stack trace to an operator who cannot act on it), `error.digest` always rendered as the only handle on the server-side stack, `console.error` on mount.

**A thrown error and a failed `QueryResult` are different paths, and must stay that way.** Most reads here return `{ok:false}` and never throw — which is why `QueryErrorCard` exists and why the `"Could not count missing audio: "` incident was fixed in `lib/db-errors.ts` rather than in a boundary. Routing `QueryResult` failures through a boundary would discard the error kind and the specific copy that goes with it.

**`notFound()` is for an absent row, never for a failed read.** Four pages converted from inline "not found" cards to `notFound()`, and in every one the `!result.ok` branch kept its `QueryErrorCard`. Collapsing the two would tell an operator their book was deleted when it is merely invisible to them behind RLS.

**`global-error.tsx` inherits nothing** — it replaces the root layout, so it declares its own `<html>`, `<body>`, `globals.css` and both font families, and its recovery is a plain `<a>`. That anchor is the one place `@next/next/no-html-link-for-pages` is wrong and is suppressed on a single line with the reasoning: `<Link>` performs a client-side navigation *inside the tree that just failed*, so it would likely land back in the same error, while a full document load is the actual recovery. Verified in the browser — the card renders with correct fonts and background, not the unstyled serif that a missing stylesheet would give.

**The Dashboard's activity card streams.** It was extracted into its own async component that runs its **own** query — a Suspense boundary only defers work that starts inside it, so a result awaited in the page and passed down as a prop would already have blocked the page. The tiles and the work queue are what an operator opens the Dashboard for; on a ~450ms-per-query database, letting the activity log resolve separately gets the useful half of the screen up a round trip sooner.

**Two things were deliberately not done.**

- **The Book editor's chapters table is not split.** It would mean lifting the table out of `BookEditor`, a client component owning the whole page through `FormProvider` — a structural happy-path change in a prompt that forbids them without asking. Flagged rather than forced; worth its own change if wanted.
