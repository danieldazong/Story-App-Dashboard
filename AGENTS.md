...
**The third tile is a ratio, not a deficit (2026-09-18).** It read `MISSING AUDIO 20` — the catalog's most important fact stated as a bare negative with no denominator, so 20-of-22 and 20-of-2000 rendered identically. It now reads `AUDIO COVERAGE 2/22` with a 4px proportion bar beneath, matching the ratio format the Books list has always used (A2). The bar is a readout of one known ratio — no axis, no series, no time dimension, nothing inferred — which is why it does not violate the no-charts rule below. A catalog with zero chapters shows `—`, never `0%` and never `NaN`.
