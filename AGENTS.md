...
- **Nested scroll container** — the Chapters table must not have its own inner scrollbar inside the page scroll. Long lists virtualise within the page scroll with a sticky table header. Never a second scrollbar. **Scoped to full-page tables (2026-09-19).** The rule is about a screen whose whole purpose is one list: there, an inner scrollbar competes with the page scroll for the same gesture over the same content. It does *not* extend to bounded cards on a composed screen — the Dashboard's `Needs attention` and `Recent activity` both scroll internally at fixed heights (304px / 360px, sticky headers), because the alternative there is not "the page scrolls this table" but "one card makes the page 2,100px tall and buries everything below it". Both cards deliberately cut a row in half at the boundary so the container reads as scrollable rather than complete.
